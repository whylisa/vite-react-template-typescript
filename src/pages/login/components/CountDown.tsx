...
const CountDown: React.FC<ICountDownParams> = ({ type, form, invitation = false }) => {
  const prefixSpan = (
   ...
   */
  function sendCode() {
    form.validateFields(['phone', 'invitation_code']).then((data: { [name: string]: any }) => {
  ...
  return (
    <>
      ...
      <Form.Item
        name="check_code"
        rules={[{ required: true, message: formatMessage({ id: 'messageNull' }) }]}
      >
        <Row gutter={6}>
          ...
          <Col className="gutter-row" span={10}>
            <Button className={messageStyles.button} disabled={!click} block onClick={sendCode}>
              {des}
            </Button>
          </Col>
        </Row>
